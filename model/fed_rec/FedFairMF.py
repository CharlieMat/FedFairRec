...
from model.fed_rec.general_fed_fair_rec import FedFairRecModel
from model.components import FieldEmbeddings, init_weights

class FedFairMF(FedFairRecModel):
    ...
    def upload_edge_params(self, local_info):
        # upload fairness statistics
        super().upload_edge_params(local_info)
        ...
        with torch.no_grad():
            ...
            self.param_proposal["uEmb.weight"][user_ids] += self.uEmb.weight.data[user_ids]
            self.param_proposal["uBias.weight"][user_ids] += self.uBias.weight.data[user_ids]
            ...
            for key in local_info:
                if "item" in key:
                    ...
                    self.param_proposal["iEmb.weight"][item_ids] += self.iEmb.weight.data[item_ids]
                    self.param_proposal["iBias.weight"][item_ids] += self.iBias.weight.data[item_ids]
    ...
    def actions_before_epoch(self, info):
        # reset fairness statistics before epoch
        super().actions_before_epoch(info)
        # reset proposal of model parameters
        self.param_proposal = {'global_bias': self.global_bias.data.clone()}
        self.param_proposal_count = {'global_bias': 0}
        for k,module in zip(['uEmb', 'uBias', 'iEmb', 'iBias'], [self.uEmb, self.uBias, self.iEmb, self.iBias]):
            self.param_proposal[k+'.weight'] = torch.zeros_like(module.weight.data)
            self.param_proposal_count[k+'.weight'] = torch.zeros(module.weight.data.shape[0]).to(self.device)
                ...
                if name in self.param_proposal:
                    agg = self.cloud_params[name] * (1-self.mitigation_alpha) + \
                            self.param_proposal[name] * self.mitigation_alpha / (self.param_proposal_count[name] \
                                if name == 'global_bias' else self.param_proposal_count[name].view(-1,1))
